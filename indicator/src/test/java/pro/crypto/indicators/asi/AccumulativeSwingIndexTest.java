...
public class AccumulativeSwingIndexTest {
    ...
    public void testASIWithLimitThree() {
        ASIResult[] result = new AccumulativeSwingIndex(buildRequest(3)).getResult();
    ...
    public void testASIWithLimitHalf() {
        ASIResult[] result = new AccumulativeSwingIndex(buildRequest(0.5)).getResult();
        assertTrue(result.length == originalData.length);
        assertNull(result[0].getIndicatorValue());
        assertEquals(result[1].getTime(), of(2018, 2, 26, 0, 0));
    ...
    private ASIRequest buildRequest(double limitMoveValue) {
