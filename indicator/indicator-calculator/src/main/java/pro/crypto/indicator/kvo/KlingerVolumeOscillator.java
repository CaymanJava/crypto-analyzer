...
import static pro.crypto.model.indicator.IndicatorType.KLINGER_VOLUME_OSCILLATOR;
...
public class KlingerVolumeOscillator implements Indicator<KVOResult> {
    ...
    private BigDecimal[] calculateExponentialMovingAverage(BigDecimal[] values, int period, int shiftCopy) {
        BigDecimal[] maValues = IndicatorResultExtractor.extractIndicatorValues(calculateMovingAverage(values, period));
