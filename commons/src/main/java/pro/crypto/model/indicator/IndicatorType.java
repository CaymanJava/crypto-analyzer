package pro.crypto.model.indicator;

import static com.google.common.collect.Sets.newHashSet;

public enum IndicatorType {

    SIMPLE_MOVING_AVERAGE,
    EXPONENTIAL_MOVING_AVERAGE,
    WEIGHTED_MOVING_AVERAGE,
    SMOOTHED_MOVING_AVERAGE,
    HULL_MOVING_AVERAGE,
    DISPLACED_MOVING_AVERAGE,
    MODIFIED_MOVING_AVERAGE,
    DOUBLE_EXPONENTIAL_MOVING_AVERAGE,
    KAUFMAN_ADAPTIVE_MOVING_AVERAGE,
    VARIABLE_INDEX_DYNAMIC_AVERAGE,
    TRIANGULAR_MOVING_AVERAGE,
    WELLES_WILDERS_MOVING_AVERAGE,
    TIME_SERIES_MOVING_AVERAGE,
    TRIPLE_EXPONENTIAL_MOVING_AVERAGE,
    MOVING_AVERAGE_CONVERGENCE_DIVERGENCE,
    COMMODITY_CHANNEL_INDEX,
    ACCUMULATION_DISTRIBUTION_LINE,
    ON_BALANCE_VOLUME,
    CHAIKIN_OSCILLATOR,
    CHAIKIN_MONEY_FLOW,
    ULTIMATE_OSCILLATOR,
    STANDARD_DEVIATION,
    AVERAGE_TRUE_RANGE,
    KELTNER_CHANNEL,
    RELATIVE_STRENGTH_INDEX,
    CONNORS_RELATIVE_STRENGTH_INDEX,
    STOCHASTIC_RELATIVE_STRENGTH_INDEX,
    LAGUERRE_RELATIVE_STRENGTH_INDEX,
    STOCHASTIC_OSCILLATOR,
    PREFERABLE_STOCHASTIC_OSCILLATOR,
    MOVING_AVERAGE_ENVELOPES,
    AVERAGE_DIRECTIONAL_MOVEMENT_INDEX,
    BOLLINGER_BANDS,
    BOLLINGER_BANDS_WIDTH,
    WILLIAMS_PERCENT_RANGE,
    AROON_UP_DOWN,
    ELDERS_FORCE_INDEX,
    RATE_OF_CHANGE,
    TRIPLE_EXPONENTIAL_AVERAGE,
    ICHIMOKU_CLOUDS,
    PARABOLIC_STOP_AND_REVERSE,
    FLOOR_PIVOT_POINTS,
    WOODIE_PIVOT_POINTS,
    CAMARILLA_PIVOT_POINTS,
    DE_MARK_PIVOT_POINTS,
    FIBONACCI_PIVOT_POINTS,
    ALLIGATOR,
    FRACTAL,
    AWESOME_OSCILLATOR,
    ACCELERATION_DECELERATION_OSCILLATOR,
    DETRENDED_PRICE_OSCILLATOR,
    AVERAGE_TRUE_RANGE_BANDS,
    CHANDELIER_EXIT,
    SWING_INDEX,
    ACCUMULATIVE_SWING_INDEX,
    CENTER_OF_GRAVITY,
    CHANDE_FORECAST_OSCILLATOR,
    CHANDE_MOMENTUM_OSCILLATOR,
    CHOPPINESS_INDEX,
    COPPOCK_CURVE,
    DISPARITY_INDEX,
    DONCHIAN_CHANNEL,
    EASE_OF_MOVEMENT,
    EHLERS_FISHER_TRANSFORM,
    ELDER_RAY_INDEX,
    GOPALAKRISHNAN_RANGE_INDEX,
    HIGH_LOW_BANDS,
    HISTORICAL_VOLATILITY,
    INTRADAY_MOMENTUM_INDEX,
    KLINGER_VOLUME_OSCILLATOR,
    MARKET_FACILITATION_INDEX,
    MASS_INDEX,
    NEGATIVE_VOLUME_INDEX,
    POSITIVE_VOLUME_INDEX,
    PRETTY_GOOD_OSCILLATOR,
    PRICE_MOMENTUM_OSCILLATOR,
    PERCENTAGE_PRICE_OSCILLATOR,
    PRICE_VOLUME_TREND,
    KNOW_SURE_THING,
    LINEAR_REGRESSION,
    QUICK_STICK,
    RAINBOW_MOVING_AVERAGE,
    RAINBOW_OSCILLATOR,
    RANDOM_WALK_INDEX,
    RELATIVE_VIGOR_INDEX,
    RELATIVE_VOLATILITY,
    SCHAFF_TREND_CYCLE,
    STOCHASTIC_MOMENTUM_INDEX,
    SUPER_TREND,
    TWIGGS_MONEY_FLOW,
    VERTICAL_HORIZONTAL_FILTER,
    VOLUME_OSCILLATOR,
    HEIKEN_ASHI,
    ELDER_IMPULSE_SYSTEM;

    public boolean isMovingAverage() {
        return newHashSet(DISPLACED_MOVING_AVERAGE, EXPONENTIAL_MOVING_AVERAGE, HULL_MOVING_AVERAGE,
                SIMPLE_MOVING_AVERAGE, SMOOTHED_MOVING_AVERAGE, WEIGHTED_MOVING_AVERAGE,
                MODIFIED_MOVING_AVERAGE, DOUBLE_EXPONENTIAL_MOVING_AVERAGE, KAUFMAN_ADAPTIVE_MOVING_AVERAGE,
                VARIABLE_INDEX_DYNAMIC_AVERAGE, TRIANGULAR_MOVING_AVERAGE, WELLES_WILDERS_MOVING_AVERAGE,
                TIME_SERIES_MOVING_AVERAGE, TRIPLE_EXPONENTIAL_MOVING_AVERAGE).contains(this);
    }

    public boolean isPivotPoint() {
        return newHashSet(FLOOR_PIVOT_POINTS, WOODIE_PIVOT_POINTS, CAMARILLA_PIVOT_POINTS,
                DE_MARK_PIVOT_POINTS, FIBONACCI_PIVOT_POINTS).contains(this);
    }

}
