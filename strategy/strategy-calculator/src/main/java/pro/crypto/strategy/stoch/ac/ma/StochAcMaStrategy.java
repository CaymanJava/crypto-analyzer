...
import static pro.crypto.model.strategy.StrategyType.STOCH_AC_MA;
...
public class StochAcMaStrategy implements Strategy<StochAcMaResult> {
    ...
    private StochResult[] stochAcResults;
    ...
    private void initResultArray() {
        result = stream(originalData)
                .map(tick -> StochAcMaResult.builder()
                        .tick(tick)
    ...
    private void calculateACStochasticFastLineCrossSignals() {
        stochAcResults = new StochResult[originalData.length];
        StochResult[] stochResults = calculateStochasticAC();
        System.arraycopy(stochResults, 0, stochAcResults, acSlowPeriod + acSmoothedPeriod - 2, stochResults.length);
        BigDecimal[] fastStochLine = extractFastStochastic();
    ...
    private BigDecimal[] extractFastStochastic() {
        return Stream.of(stochAcResults)
                .map(this::extractFastStochasticResult)
    ...
    private void addIndicatorsResult(int index) {
        StochAcMaResult stochAcMaResult = result[index];
        addAcResult(acResults[index], stochAcMaResult);
        addStochAcResult(stochAcResults[index], stochAcMaResult);
        addMaResult(maResults[index], stochAcMaResult);
    }
    ...
    private BigDecimal extractSlowStochasticResult(StochResult result) {
        return ofNullable(result).map(StochResult::getSlowStochastic).orElse(null);
    }

    private BigDecimal extractFastStochasticResult(StochResult result) {
        return ofNullable(result).map(StochResult::getFastStochastic).orElse(null);
    }
