...
public class PPOAnalyzer implements Analyzer<PPOAnalyzerResult> {

    private final Tick[] originalData;
    private final PPOResult[] indicatorResults;

    private BigDecimal[] indicatorValues;
    private PPOAnalyzerResult[] result;

    public PPOAnalyzer(AnalyzerRequest request) {
        this.originalData = request.getOriginalData();
        this.indicatorResults = (PPOResult[]) request.getIndicatorResults();
    }
    ...
    public void analyze() {
        ...
        buildPPOAnalyzerResult(mergedSignals);
    }

    @Override
    public PPOAnalyzerResult[] getResult() {
        if (isNull(result)) {
            analyze();
        }
        return result;
    ...
    private void buildPPOAnalyzerResult(SignalStrength[] mergedSignals) {
        result = IntStream.range(0, indicatorResults.length)
                .mapToObj(idx -> new PPOAnalyzerResult(indicatorResults[idx].getTime(), mergedSignals[idx]))
                .toArray(PPOAnalyzerResult[]::new);
    }
