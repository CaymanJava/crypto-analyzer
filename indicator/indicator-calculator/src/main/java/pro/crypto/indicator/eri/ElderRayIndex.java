...
import static pro.crypto.model.indicator.IndicatorType.ELDER_RAY_INDEX;
...
public class ElderRayIndex implements Indicator<ERIResult> {
    ...
    private BigDecimal[] calculateMovingAverageValues() {
        return IndicatorResultExtractor.extractIndicatorValues(calculateExponentialMovingAverage());
    }
    ...
    private BigDecimal[] calculateSignalLineValues(BigDecimal[] indicatorValues) {
        BigDecimal[] signalLineValues = IndicatorResultExtractor.extractIndicatorValues(calculateSimpleMovingAverage(indicatorValues));
    ...
    private BigDecimal[] calculateSmoothedLineValues(BigDecimal[] indicatorValues) {
        BigDecimal[] smoothedLineValues = IndicatorResultExtractor.extractIndicatorValues(calculateSmoothedMovingAverage(indicatorValues));
        BigDecimal[] result = new BigDecimal[originalData.length];
